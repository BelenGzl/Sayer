...
<form action =  "{{route('guardatrabajador')}}" method = "POST" enctype="multipart/form-data">

<table>
<th>Registrar trabajador</th>
{{csrf_field()}}

<tr><td>
@if($errors->first('id_Tra')) 
<i> {{ $errors->first('id_Tra') }} </i> 
@endif	
Clave</td><td><input type = 'text' name = 'id_Tra' value="{{($idtra)}}" readonly='readonly'>
</td></tr>
